...
class HotelReservationAmendment(models.Model):
    ...
    def _apply_financial_effects(self, before, after):
        ...
        self.ensure_one()
        difference = after["amount_total"] - before["amount_total"]
        currency = self.reservation_id.currency_id
...
class HotelReservationGroup(models.Model):
    ...
    def action_create_group_invoice(self):
        ...
        if len(lines.mapped("currency_id")) != 1:
            raise UserError(
                _("Create a separate group invoice for each folio currency.")
            )
        invoice = self.env["account.move"].create(
            {
                "move_type": "out_invoice",
                "partner_id": self.billing_partner_id.id,
                "currency_id": lines[:1].currency_id.id,
        ...
        for line, invoice_line in zip(lines, invoice.invoice_line_ids):
            line._link_invoice_line(invoice_line)
        return {
            "name": _("Group Invoice"),
