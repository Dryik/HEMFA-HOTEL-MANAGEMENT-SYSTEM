...
- The active Odoo company is one hotel. A separate hotel is represented by a separate Odoo company.
...
- The hotel business day is company-local noon to noon and is converted to UTC by one shared helper.
...
- `hotel_board` is the final staff-workspace composition addon. Its Dashboard and
  ...
  hotel state.
- The primary reservation planner is a complete-room Owl inventory tape. The native
  Reservation Planner remains a fallback; reservation and housekeeping state changes
